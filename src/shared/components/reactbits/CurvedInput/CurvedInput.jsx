...
// React Bits — CurvedInput (adaptado a JSX/JS).
// Input cuyo texto se dibuja a lo largo de una curva (SVG); un <input> nativo
// oculto captura el foco/teclado y sincroniza caret y selección.
...
// Mapea el espacio plano (u a lo largo de la barra, v desde la línea central)
// sobre un arco circular con la sagita dada (`bend`, en px).
...
const CurvedInput = ({
  ...
  // Re-medir cuando terminan de cargar las webfonts
  useEffect(() => {
    let alive = true;
  ...
  // Medir el texto renderizado para mantener el caret sobre la curva y hacer
  // scroll de valores largos a lo largo del arco, como un input nativo. Corre a
  // propósito en cada render (sin deps) para reaccionar a cualquier cambio de medida.
  ...
  // Click sobre la curva: enfoca el input oculto y coloca el caret en el
  // carácter más cercano al click, medido en longitud de arco.
  const handleSurfaceClick = (e) => {
    ...
    try {
      ...
      /* selection API no disponible para este tipo de input */
    }
    setCaretIndex(idx);
  };
